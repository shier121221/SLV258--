...
{	
	...
	RCC->APB2ENR |= 1<<0;
	JTAG_Set(SWD_ENABLE); // 禁用JTAG，保留SWD调试接口

	// PA0-PA6: DB0-DB6, PA8: RS, PA9: WR, PA10: RD, PA11: RST, PA12: DB7
	GPIOA->CRL &= 0xF0000000;
	GPIOA->CRL |= 0x03333333; // PA0-PA6 推挽输出（PA7保留浮空）
	GPIOA->CRH &= 0xFFF00000; // 保留PA13-PA15（SWD）
	GPIOA->CRH |= 0x00033333; // PA8-PA12 推挽输出
	GPIOA->ODR |= 0x1F00;     // RS/WR/RD/RST/DB7 初始高

	// PB3-PB10: DB8-DB15
	GPIOB->CRL &= 0x00000FFF;
	GPIOB->CRL |= 0x33333000; // PB3-PB7 推挽输出
	GPIOB->CRH &= 0xFFFFF000;
	GPIOB->CRH |= 0x00000333; // PB8-PB10 推挽输出
	GPIOB->ODR |= 0x07F8;     // DB8-DB15 初始高

	// PC14: LED 背光
	GPIOC->CRH &= 0xF0FFFFFF;
	GPIOC->CRH |= 0x03000000; // PC14 推挽输出
...
u16 LCD_Read_ID(u8 reg)
{
	...
	LCD_RD_DATA();   	    	//����0X00
	lcddev.id=LCD_RD_DATA();   	//��ȡ85								   
	lcddev.id<<=8;
	lcddev.id|=LCD_RD_DATA();  	//��ȡ52 	
	return lcddev.id;
}
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	GPIOB->CRH=0X33333333; 		//PB8-15 �������
	GPIOB->ODR=0XFFFF;    		//ȫ�������  
	return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//��ʽת��һ��
}		
