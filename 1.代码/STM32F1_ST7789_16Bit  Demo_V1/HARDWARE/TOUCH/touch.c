...
#define TP_CALIB_FLASH_ADDR   0x0800FC00u
#define TP_CALIB_MAGIC        0x5443u
#define TP_CALIB_MARK         0xA5u

static void TP_Flash_WaitDone(void)
{
	while (FLASH->SR & FLASH_SR_BSY);
}

static void TP_Flash_ClearStatus(void)
{
	FLASH->SR |= FLASH_SR_EOP | FLASH_SR_PGERR | FLASH_SR_WRPRTERR;
}

static void TP_Flash_Unlock(void)
{
...
static void TP_Flash_Lock(void)
{
	FLASH->CR |= FLASH_CR_LOCK;
}

static void TP_Flash_ErasePage(u32 addr)
{
	TP_Flash_WaitDone();
	TP_Flash_ClearStatus();
...
static void TP_Flash_ProgramHalfWord(u32 addr, u16 data)
{
	TP_Flash_WaitDone();
	TP_Flash_ClearStatus();
...
void TP_Save_Adjdata(void)
{
	...
	buf[0] = (u8)(TP_CALIB_MAGIC & 0xFF);
	buf[1] = (u8)(TP_CALIB_MAGIC >> 8);
	temp = (s32)(tp_dev.xfac * 100000000.0f);
	buf[2] = temp;
	buf[3] = temp >> 8;
	buf[4] = temp >> 16;
	buf[5] = temp >> 24;
	temp = (s32)(tp_dev.yfac * 100000000.0f);
	buf[6] = temp;
	buf[7] = temp >> 8;
	buf[8] = temp >> 16;
	buf[9] = temp >> 24;
	buf[10] = tp_dev.xoff;
	buf[11] = tp_dev.xoff >> 8;
	buf[12] = tp_dev.yoff;
	buf[13] = tp_dev.yoff >> 8;
	buf[14] = tp_dev.touchtype;
	buf[15] = TP_CALIB_MARK;

	TP_Flash_Unlock();
	TP_Flash_ErasePage(TP_CALIB_FLASH_ADDR);
	for (i = 0; i < 16; i += 2) {
		hw = (u16)buf[i] | ((u16)buf[i + 1] << 8);
		TP_Flash_ProgramHalfWord(TP_CALIB_FLASH_ADDR + i, hw);
	}
	TP_Flash_Lock();
}
...
{
	const volatile u8 *buf = (const volatile u8 *)TP_CALIB_FLASH_ADDR;
	s32 tempfac;
	u16 magic;
	u8 touchtype;

	magic = (u16)buf[0] | ((u16)buf[1] << 8);
	if ((magic != TP_CALIB_MAGIC) || (buf[15] != TP_CALIB_MARK))
		return 0;
	...
	tempfac <<= 8;
	tempfac |= buf[2];
	tp_dev.xfac = (float)tempfac / 100000000.0f;
	...
	tp_dev.xoff = (short)((u16)buf[10] | ((u16)buf[11] << 8));
	tp_dev.yoff = (short)((u16)buf[12] | ((u16)buf[13] << 8));
	touchtype = buf[14];

	if (tp_dev.xfac <= 0.0f || tp_dev.xfac >= 1.0f)
		return 0;
	if (tp_dev.yfac <= 0.0f || tp_dev.yfac >= 1.0f)
		return 0;
	if (tp_dev.xoff < -1000 || tp_dev.xoff > 1000)
		return 0;
	if (tp_dev.yoff < -1000 || tp_dev.yoff > 1000)
		return 0;
	if (touchtype & 0X7E)
		return 0;
